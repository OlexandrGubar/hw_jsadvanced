...
function init() {
    form1.userName.addEventListener('change', function nameOnChange() {
        var pattern = /\S/;
        validate(this, pattern);
    })
    form1.email.addEventListener('change', function emailOnChange() {
        var pattern = /\b[a-z0-9._]+@[a-z0-9.-]+\.[a-z]{2,4}\b/i;
        validate(this, pattern);
    } )
    form1.zip.addEventListener('change', function zipcodeOnChange() {
        var pattern = /\d{5}/;
        validate(this, pattern);
    } )
    form1.addEventListener('submit', function onsubmitHandler(event) { 
